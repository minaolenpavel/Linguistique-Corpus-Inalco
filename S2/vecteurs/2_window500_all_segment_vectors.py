...
# sauvegarde de la matrice de cosinus dans un fichier au format csv
with open( 'JV80j_cosinus_matrix.csv', 'w' ) as out_matrix:
     for i in range( 0, len( all_segment_vectors)):
          for j in range( 0, len( all_segment_vectors )):
               out_matrix.write( '{0}'.format( cosine_matrix[ i ][ j ] ))
               if j < (len( all_segment_vectors ) - 1):
                    out_matrix.write( '\t' )
               else:
                    out_matrix.write( '\n' )
